...
// ── The big transactional company creation ────────────────────────────────────

/**
 * Create the full company from the wizard payload (§D.2) inside one transaction.
 * Validation (§C) runs first via assertOnboardingPayload + DB uniqueness checks.
 * Does NOT send the welcome email (controller does) but RETURNS the data needed for it.
 *
 * @returns {object} { company, admin, subscription, invoice, contact, billing, modules,
 *                      approval, branding, wallet, welcome }
 */
async function createCompany(payload, actorId, { sendWelcomeEmail } = {}) {
  ...
  // 2. DB uniqueness pre-checks (rules 3, 11, 13) for precise 409 messages.
  ...
  // Temp password (returned ONCE so the operator can hand it over).
  const tempPassword =
  ...
  const result = await withTransaction(async (client) => {
    // ── companies (incl. extended onboarding profile + address columns) ──────
    const { rows: companyRows } = await client.query(
      `INSERT INTO companies
         (name, code, legal_name, registration_number, gstin, pan, industry, website,
          ...
          pincode, country, timezone, currency, logo_url, status, created_by)
      ...
      [
        ...
        null, // legacy free-text address column left null (prefer structured fields)
    ...
    // ── company_contacts (one row from contact) ──────────────────────────────
    let contactRow = null
    ...
    // ── company_admins (hashed temp password, username, force_password_reset) ─
    ...
    // ── company_subscriptions (new fields + legacy plan via §A.3 map) ────────
    const { rows: subRows } = await client.query(
      ...
      [
        companyId,
        legacyPlan,
        amounts.total_amount, // legacy 'amount' mirrors the computed total
        currency,
        startDate,
        endDate,
        planTier,
    ...
    // ── company_wallets (controls + limits) ──────────────────────────────────
    ...
    const { rows: walletRows } = await client.query(
      ...
      [
        ...
        money(wallet.credit_limit),
        money(wallet.low_balance_threshold),
        !!wallet.auto_recharge_enabled,
      ]
    )
    const walletRow = walletRows[0]

    // Record the initial allocation as a wallet transaction when balance > 0.
    if (initialBalance > 0) {
      await client.query(
    ...
    // ── company_settings (legacy per-module toggles + approval/reports) ──────
    ...
    // ── company_modules (one row per module) ─────────────────────────────────
    const moduleRows = []
    ...
    const { rows: approvalRows } = await client.query(
      ...
      [
        companyId,
        !!approval.approval_required,
        approval.approval_type || 'none',
        JSON.stringify(Array.isArray(approval.levels) ? approval.levels : []),
        actorId,
      ]
    )
    ...
    // ── company_invoices (built via invoice.service.buildInvoiceRecord) ──────
    const invoiceRecord = invoiceService.buildInvoiceRecord({
    ...
    const { rows: invoiceRows } = await client.query(
      `INSERT INTO company_invoices
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,
               COALESCE($9,'issued'),$10,$11,COALESCE($12, now()),$13)
       RETURNING *`,
      [
        companyId,
        invoiceRecord.invoice_number,
        subscriptionRow.id,
        money(invoiceRecord.base_amount),
        money(invoiceRecord.tax_amount),
        money(invoiceRecord.discount_amount),
        money(invoiceRecord.total_amount),
        invoiceRecord.currency || currency,
        invoiceRecord.status || null,
        invoiceRecord.due_date || null,
        JSON.stringify(invoiceRecord.line_items || []),
        invoiceRecord.issued_at || null,
        actorId,
      ]
    )
    const invoiceRow = invoiceRows[0]

    // ── If submitted from a draft, mark it submitted + link the company ──────
    if (draftId) {
      await client.query(
        `UPDATE onboarding_drafts SET status = 'submitted', company_id = $1, updated_at = now()
          WHERE id = $2 AND created_by = $3`,
        [companyId, draftId, actorId]
      )
    }
  ...
  // Assemble the response object. temp_password is returned ONCE (not persisted in clear).
  return {
    ...
    // Data the controller needs to send the welcome email (it owns delivery).
    welcome: {
